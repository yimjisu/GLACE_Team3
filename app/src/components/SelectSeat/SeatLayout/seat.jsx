...
export class Seat {
  ...
  animate(ctx) {
    ...
    this.finalPos = this.startPos
      .clone()
      .add(this.pos.clone().multiply(this.zoomIndex));
    ...
    if (this.isReserved && !this.isSelected) {
      ...
      ctx.beginPath();
      ctx.moveTo(
        this.finalPos.x + this.finalWidth * 0.1,
        this.finalPos.y + this.finalHeight * 0.1
      );
      ctx.lineTo(
        this.finalPos.x + this.finalWidth * 0.9,
  ...
  move(point) {
    if (this.isDown) {
      this.movePos = this.downStartPos
        .clone()
        .add(point)
  ...
  up(point) {
    ...
    if (
      !this.isMove &&
      point.collide(this.finalPos, this.finalWidth, this.finalHeight)
    ) {
      return this;
    } else {
      this.isMove = false;
