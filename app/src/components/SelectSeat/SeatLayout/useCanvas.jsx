    ...
    for (let i = 0; i < allSeats.length; i++) {
      ...
      for (let j = 0; j < tempSeatReservationInfo.length; j++) {
        ...
        if (name == allSeats[i].seatName) {
          ...
          tempSeatReservationInfo = tempSeatReservationInfo.filter(
            (item) => item != name
          );
          reserved = true;
          break;
    ...
    if (selectIndex != null) {
      ...
      if (seat.isSelected) {
        try {
          ...
          let newSeatReservationInfo = seatReservationInfo.filter(
            (item) => item != seat.seatName
          );
          setSeatReservationInfo(newSeatReservationInfo);
          newSelectedSeat = newSelectedSeat.filter(
            (item) => item != seat.seatName
          );
